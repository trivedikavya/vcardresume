...
async function run() {
  ...
  const owner = "trivedikavya";
  const repo = "vcardresume";
  ...
  await octokit.issues.addLabels({
    ...
    labels: [`level:${data.level.toLowerCase()}`]
  });

  // 2. Post Greeting Message
  const message = data.is_ignore 
    ? `Thanks for raising this! Marked as out-of-scope. Reason: ${data.reason}`
